...
# ── Load Data ─────────────────────────────────────────────────
df = pd.read_csv(os.path.join(DATA_DIR, 'india_housing.csv'))
...
plt.savefig(os.path.join(FRONTEND_STATIC, 'eda_plots.png'), dpi=120, bbox_inches='tight', facecolor='#0f172a')
plt.close()
...
if hasattr(best_model, 'feature_importances_'):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(FRONTEND_STATIC, 'feature_importance.png'), dpi=120, bbox_inches='tight', facecolor='#0f172a')
    plt.close()
...
for spine in ax.spines.values(): spine.set_edgecolor('#334155')
plt.tight_layout()
plt.savefig(os.path.join(FRONTEND_STATIC, 'actual_vs_predicted.png'), dpi=120, bbox_inches='tight', facecolor='#0f172a')
plt.close()
...
city_stats.to_csv(os.path.join(DATA_DIR, 'city_stats.csv'), index=False)
